...
import UncertainSCI.opolynd as opolynd
...
class PolysTestCase(unittest.TestCase):
    ...
    def test_tensor_gauss_quadrature(self):
        ...
        Inds = indexing.LpSet(dim=dim, order=N - 1, p=np.inf)

        J = [None,] * dim
        for q in range(dim):
            J[q] = families.JacobiPolynomials(alpha=alpha, beta=beta)

        P = opolynd.TensorialPolynomials(polys1d=J)
