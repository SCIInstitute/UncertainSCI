...
import UncertainSCI.utils.quad as quad
...
class QuadTestCase(unittest.TestCase):
    ...
    def test_gq_modification_global(self):
        """Test gq_modification for a global integral.
            ...
            for m in range(N):
                def integrand(x):
                    return (
                        J.eval(x, m).flatten() *
                        J.eval(x, n).flatten() *
                        families.jacobi_weight_normalized(x, alpha, beta)
                    )

                G[n, m] = quad.gq_modification(
                    integrand,
                    -1,
                    1,
                    math.ceil(1 + (n + m) / 2.),
    ...
    def test_gq_modification_composite(self):
        """Test gq_modification using a composite strategy.
        ...
        # Integrate the weight function and use modifications for the polynomial part.
        def integrand(x):
            return families.jacobi_weight_normalized(x, alpha, beta)
